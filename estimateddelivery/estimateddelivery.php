...
class EstimatedDelivery extends Module
{
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitEstimatedDeliveryConfig')) {
            ...
            if ($deliveryDays < 1) {
                ...
                // Validazione holidays
                $validatedHolidays = $this->validateHolidays($holidays);
                ...
                Configuration::updateValue('ESTIMATED_DELIVERY_COUNTDOWN_TEXT', pSQL($countdownText));
                Configuration::updateValue('ESTIMATED_DELIVERY_AFTER_HOURS_TEXT', pSQL($afterHoursText));
    ...
    /**
     * Validazione date festività
     */
    private function validateHolidays($input)
    {
        $valid = [];
        $dates = preg_split('/[\s,\n\r]+/', $input);
            ...
            'form' => [
                ...
                'input' => [
                    ...
                    [
                        'type' => 'html',
                        'name' => 'separator_3',
                        'html_content' => '<hr><h4 style="color: #00897B;">💡 Come Utilizzare il Modulo</h4>' .
                            '<div style="background: #f0f9ff; padding: 15px; border-left: 4px solid #00897B; margin: 10px 0;">' .
                            '<p><strong>Nel tuo template Smarty (.tpl), inserisci:</strong></p>' .
                            '<code style="background: white; padding: 8px; display: block; margin: 10px 0;">' .
                            '{hook h=\'displayEstimatedDelivery\'}' .
                            '</code>' .
                            '<p style="margin-top: 10px;"><strong>Esempio posizionamento in product.tpl:</strong></p>' .
                            '<pre style="background: white; padding: 10px; overflow-x: auto;">' .
                            htmlspecialchars('<div class="product-delivery-info">
    {hook h=\'displayEstimatedDelivery\'}
</div>') .
                            '</pre>' .
                            '</div>',
    ...
    {
        return $this->renderDeliveryInfo('displayEstimatedDelivery');
    }

    /**
     * Hook standard PrestaShop - Sotto il prezzo del prodotto
     */
    public function hookDisplayProductAdditionalInfo($params)
    {
        return $this->renderDeliveryInfo('displayProductAdditionalInfo');
    }

    /**
     * Hook standard PrestaShop - Zona rassicurazione
     */
    public function hookDisplayReassurance($params)
    {
        return $this->renderDeliveryInfo('displayReassurance');
    }

    /**
     * Hook standard PrestaShop - Vicino ai pulsanti acquisto
     */
    public function hookDisplayProductButtons($params)
    {
        return $this->renderDeliveryInfo('displayProductButtons');
    }

    /**
     * Hook standard PrestaShop - Dopo il nome del prodotto
     */
    ...
    public function hookHeader()
    {
        ...
        if (Configuration::get('ESTIMATED_DELIVERY_COUNTDOWN_ENABLED')) {
            $this->context->controller->registerJavascript(
                'modules-estimateddelivery-countdown',
                'modules/' . $this->name . '/js/countdown.js',
